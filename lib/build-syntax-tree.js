...
export default function buildSyntaxTree(options, tokens) {
    ...
    for (const token of tokens) {
        if (token.tokenString === '{{') {
            if (mustacheOpenToken) {
                throw new LineSyntaxError(`Failed to close mustache opened on line ${ mustacheOpenToken.lineNumber }`, mustacheOpenToken);
            } else {
                mustacheOpenToken = token;
            }
        } else if (token.tokenString === '{{!--') {
            if (commentOpenToken) {
                throw new LineSyntaxError(`Failed to close comment opened on line ${ commentOpenToken.lineNumber }`, commentOpenToken);
            } else {
    ...
    if (blocks.length > 0) {
        ...
        throw new LineSyntaxError(`Failed to close block opened on line ${ token.lineNumber }`, token);
    }
    ...
    function addExpression(openToken, closeToken, parts) {
        ...
        function pushTokenString(str) {
            ...
            if (str.startsWith('#')) {
                pushBlockOpen({
                    type: 'BLOCK_OPEN',
                    exp: parseExpression(openToken, closeToken, str.slice(1).trim()),
                    children: [],
                    tokens: parts,
                });
                ...
                pushNode(createExpressionNode(openToken, closeToken, parts, str));
            }
        }
    ...
    function parseExpression(openToken, closeToken, tokenString) {
        ...
        if (inBracket) {
            throw new LineSyntaxError(`Unclosed bracket "[...]" in expression starting on line ${ openToken.lineNumber }`, openToken);
        }
        if (inBlockParams) {
            throw new LineSyntaxError(`Unclosed block params in expression starting on line ${ openToken.lineNumber }`, openToken);
        }
        if (inStringLiteral) {
            throw new LineSyntaxError(`Unclosed string literal in expression starting on line ${ openToken.lineNumber }`, openToken);
        }
        if (inStartKeyValue) {
            throw new LineSyntaxError(`No value defined for key/value after "=" in expression starting on line ${ openToken.lineNumber }`, openToken);
        }
    ...
    function parseReferencePath(openToken, closeToken, refpath) {
        if (refpath[0] === '.') {
            throw new LineSyntaxError(
                `A reference path cannot start with "." in expression starting on line ${ openToken.lineNumber }`,
                openToken
            );
        }
        ...
        for (; charIndex < refpath.length; charIndex += 1) {
            ...
            if (inBracket) {
                ...
                throw new LineSyntaxError(
                    `Invalid JavaScript symbol character "${ c }" in expression starting on line ${ openToken.lineNumber }`,
                    openToken
                );
            }
        }
    ...
    function validateKeyValueKey(openToken, closeToken, key) {
        if (/^[\W]+$/.test(key)) {
            throw new LineSyntaxError(
                `Invalid characters in key "${ key }" in expression starting on line ${ openToken.lineNumber }`,
                openToken
            );
        }
        return key;
