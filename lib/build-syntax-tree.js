...
export default function buildSyntaxTree(options, tokens) {
    ...
    function addExpression(parts, openToken) {
        // The "parts" are the tokens representing a single expression which was split over more than 1 line.
        const tokenString = parts.map((t) => t.tokenString).join(' ');
        const lastToken = parts[parts.length - 1];

        function pushTokenString(str) {
            str = str.trim();
            // Some expressions are explicitly typed using the first few characters.
            if (str.startsWith('#')) {
                pushBlockOpen({
                    type: 'BLOCK_OPEN',
                    exp: parseExpression(openToken, lastToken, str.slice(1).trim()),
                    children: [],
                    tokens: parts,
                });
                ...
                pushNode({ type: 'ELSE', tokens: parts });
                // Push the remaining part of the else ... if (or other statement)
                pushTokenString(str.slice(4));
            } else {
                pushNode(createExpressionNode(parts, openToken, tokenString));
            }
        }
    ...
    function createExpressionNode(parts, openToken, tokenString) {
        const lastToken = parts[parts.length - 1];
        const exp = parseExpression(openToken, lastToken, tokenString);
    ...
    function parseExpression(openToken, lastToken, tokenString) {
        ...
        function closeSpaceDelineatedSubToken() {
            if (subToken && subToken.type === 'KEY_VALUE') {
                if (inStringLiteral) {
                    ...
                    pushExpressionToken({
                        value: parseSymbols(openToken, lastToken, expressionTokenString),
                    });
                }
            } else if (inStringLiteral) {
                expressionTokens.push({ type: 'LITERAL', value: stringLiteral });
            } else if (expressionTokenString) {
                expressionTokens.push(parseSymbols(openToken, lastToken, expressionTokenString));
            }
        }
        ...
        for (; charIndex < tokenString.length; charIndex += 1) {
            ...
            if (c === '=') {
                ...
                if (lastSubToken && lastSubToken.type === 'PATH') {
                    key = validateKeyValueKey(openToken, lastToken, lastSubToken.pathString);
                    // Throw away the last PATH since we converted it to the key in key/value.
                    expressionTokens.pop();
                } else {
                    key = validateKeyValueKey(openToken, lastToken, expressionTokenString);
                }
        ...
        if (inBracket) {
            throw new LineSyntaxError(`Unclosed bracket "[...]" on line ${ openToken.lineNumber }`, openToken);
        }
        if (inBlockParams) {
            throw new LineSyntaxError(`Unclosed block params on line ${ openToken.lineNumber }`, openToken);
        }
        if (inStringLiteral) {
            throw new LineSyntaxError(`Unclosed string literal on line ${ openToken.lineNumber }`, openToken);
        }
        if (inStartKeyValue) {
            throw new LineSyntaxError(`No value defined for key/value after "=" on line ${ openToken.lineNumber }`, openToken);
        }
    ...
    function parseSymbols(openToken, lastToken, expressionString) {
        ...
        function castToNumberOrReferencePath(str) {
            ...
            rv.type = 'PATH';
            rv.path = parseReferencePath(openToken, lastToken, str);
            rv.pathString = str;
            return rv;
        }
    }

    function parseReferencePath(openToken, lastToken, refpath) {
        if (refpath[0] === '.') {
            throw new LineSyntaxError(
                `A reference path cannot start with "." in path expression on line ${ openToken.lineNumber }`,
                openToken
            );
        }
        ...
        for (; charIndex < refpath.length; charIndex += 1) {
            ...
            if (inBracket) {
                ...
                throw new LineSyntaxError(
                    `Invalid JavaScript symbol character "${ c }" in path expression on line ${ openToken.lineNumber }`,
                    openToken
                );
            }
        }
    ...
    function validateKeyValueKey(openToken, lastToken, key) {
        if (/^[\W]+$/.test(key)) {
            throw new LineSyntaxError(`Invalid characters in key "${ key }" on line ${ openToken.lineNumber }`, openToken);
        }
        return key;
    }
